...
int uthreshold = 340;
int lthreshold = 330;
bool lowerpassed = false;

unsigned int beats = 0; 

unsigned long starts;
unsigned long currents;
...
void loop() {
  ...
  if (Signal <= lthreshold){
    lowerpassed = true;
  }

  if (Signal >= uthreshold && lowerpassed == true) {
    beats++;
    lowerpassed = false; 
  }
  else {
    Signal = (lthreshold + uthreshold) / 2; // If signal is out of bounds, set to the average of the bounds
  }
  ...
  int lower_bound = 310;                      //minimum signal values
  int upper_bound = 360;                   //Maximum signal values
